...
PIPELINE (LEAKAGE-FREE):
  ...
  4.5 RandomizedSearchCV tuning for each candidate
...
def print_metrics_table(metrics_by_model):
    ...
    for name, m in ordered:
        ...
        print(f"  {name:<24} {auc:>9} {m.get('f1', 0):>9.4f}")
...
def evaluate_composite_pipeline_oof(best_base_model, X, y, grey_features, random_state):
    ...
    for fold_i, (tr_idx, va_idx) in enumerate(cv.split(X, y), 1):
        ...
        mask_conf_cons = (ens_tr < GREY_ZONE_LOW) & (y_tr == 0)  # ✅ исправлен баг
...
def run_pipeline():
    ...
    # ── Step 4.5: RandomizedSearchCV tuning ──────────────────────────────────
    ...
    for name, model in candidates.items():
        if name not in grids:
            print(f"  [{name}] no grid defined — skipping tuning")
        ...
        tuning_summary[name] = {
            ...
            "best_cv_auc": round(search.best_score_, 6),
        }
        print(f"    best CV AUC: {search.best_score_:.6f}")
        print(f"    best params: {search.best_params_}")

    # ── Step 5 & 6: OOF evaluation ────────────────────────────────────────────
    ...
    for name, model_candidate in candidates.items():
        ...
        metrics_by_model[name]    = calculate_metrics(
            y, (final_oof_scores >= 0.5).astype(int), final_oof_scores
        )
    ...
    mask_conf_cons = (ensemble_scores_all < GREY_ZONE_LOW) & (y == 0)  # ✅ исправлен баг
    ...
    plot_confusion_matrix(y, best_pred,
        FIGURES_DIR / "01_confusion_matrix.png",
        "Confusion Matrix — Composite Pipeline OOF")
    plot_score_distribution(
        {"business (OOF)": final_oof_scores[y == 1],
         "consumer (OOF)": final_oof_scores[y == 0]},
    ...
    plot_consumer_score_breakdown(
        meta_oof["base_scores"], meta_oof["dist_scores"],
        meta_oof["iso_scores"], final_oof_scores,
